...
### 跨日日程（iOS/iPadOS，build 16 显示与编辑修复）
...
这是一个普通活动时间戳区间，不是 Scheduled 到 Deadline。Org Agenda 会在 10、11、12、13 日显示它；Aster 的修复也遵循这个日期范围。时间轴按当天实际占用时段裁切，结束于午夜时不会凭空增加该日的占用时间。

详情保留开始和结束的完整日期与时刻。只修改标题后，以上时间戳应原样保留；开启全天后应是 `<2026-09-10 Thu>--<2026-09-13 Sun>`。没有结束时间的单点 Event 仍合法，不会自动补一个结束时间。

新建和详情中的开始、结束日期使用相同的本地化格式，并显示完整年份。点击日期会就地弹出日历，不进入子页面；时刻紧跟在日期后，沿用系统的 12/24 小时制。

### 日期范围内每天同一时段（iOS/iPadOS，build 16）
...
这表示 11、12、13 日每天 19:00–22:00，不是连续三天占用。Agenda 会分别标出 `(1/3)`、`(2/3)`、`(3/3)`，时间轴只绘制当天的三小时。详情编辑仍写回这一个 Org 标题，保留两端的时间段；只改标题不会改动时间戳。

这是 Org 官方的[日期范围内每日时段](https://orgmode.org/manual/Timestamps.html)语法，不需要添加重复规则或拆成三条日程。

如果每天的时段是 `22:00-01:00`，Aster 还会显示上一晚延续到当天凌晨的部分。中间日期同时显示凌晨和晚间两段，最后一晚的凌晨部分也不会丢失。结束于午夜时，不会多画一个空时段。

这些显示片段仍属于同一个 Org 标题。进度按写出的三个开始日期计数，最后的凌晨部分仍属于 `(3/3)`。保存开始、结束时刻相同的时段，也不会擅自把 Org 的结束日期再推后一天。
...
未发布更新：以 9 月 10 日为例，同一行会同时显示「计划已过 3 天」和「截止 · 距到期 1 天」，而不是用计划日期的过期状态遮住截止日期。
...
iOS/iPadOS build 16 按 Org 的日期进位计算重复，不将无效日期压到月末。例如 2027 年 1 月 31 日加一个月是 3 月 3 日。Agenda 从原日期逐期展开，因此下一项仍可能是 3 月 31 日；完成任务会按重复模式改变源日期基准。

修改日期和 Repeat 会直接更新原 Org 条目，不生成单次副本，也不会额外自动推进一期。Habit 编辑保留完成历史；明确执行完成操作时，仍按正常流程记录日志。
