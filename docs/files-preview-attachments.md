...
未发布修复还会正确识别 Markdown 标题中的制表符和末尾 `#` 标记。GFM 表格内没有竖线的普通行仍是单元格，遇到空行或其他块结构才结束表格。紧接表格的代码块无论使用 Tab 还是等价空格缩进，都独立显示，不会变成表格行。源文本保持不变。
...
添加附件前会检查实际目标仍在工作区内，包括已有的符号链接目录；越界时会报错，不复制文件。这项未发布修复不要求 ID 必须是 UUID，安全的自定义 Org ID 仍然可用。
