...
iOS/iPadOS build 16 支持[选择同步子目录](sync-and-safety.md)。被排除的目录中，已有本地文件仍可打开；仅在云端的文件会提示先在设置中重新勾选。Files 中主动确认的云端文件操作与自动同步范围是两回事。
...
iOS/iPadOS build 15 正确识别 Markdown 标题中的制表符和末尾 `#` 标记。GFM 表格内没有竖线的普通行仍是单元格，遇到空行或其他块结构才结束表格。紧接表格的代码块无论使用 Tab 还是等价空格缩进，都独立显示，不会变成表格行。源文本保持不变。

Org 文本和正则链接搜索也已修正 CRLF 换行的行号计算；相同内容使用 LF 或 CRLF 时会跳到同一行。
...
iOS/iPadOS build 15 添加附件前会检查实际目标仍在工作区内，包括已有的符号链接目录；越界时会报错，不复制文件。这不要求 ID 必须是 UUID，安全的自定义 Org ID 仍然可用。

移动整个 Org 文件不会自动迁移旁边的附件目录。跨目录移动整个文件时，仍需一起维护相对路径和 `data/`；这与 Aster 的标题子树移动不是同一个操作。
