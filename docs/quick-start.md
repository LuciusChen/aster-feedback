...
iPhone 上三组控件位于同一条底栏；一级 Tab 的可用空间足够宽时显示图标和文字，包括 iPhone 15 在内的较紧凑宽度会只显示图标，三个入口的选中状态和无障碍名称仍然独立可用，不会互相重叠。iPad 上二级按钮位于整个窗口左侧，一级 Tab 严格相对整个窗口居中，新建和搜索位于最右侧；三组控件共用同一个垂直中心，也不会增加全局侧边栏。
