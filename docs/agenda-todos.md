...
- 轻点标题或其他区域：打开同一个详情页。关键字的点击范围紧贴可见胶囊，不向标题区域扩张。iPhone 使用底部面板，iPad 使用锚定在胶囊旁的 popover；选中后立即写入并关闭，不需要再点 Done。非重复条目切到 Terminal 后会在原位置短暂停留以确认新状态，然后才从活动列表消失。
- 长按 Task：只打开 Schedule、Move/Archive、Note 和 Delete 等条目操作，不再重复显示关键字列表。
