...
Journal 与 Agenda 共用同一个会持久化的 Week/Month/Year 日历尺寸，也共用 **设置 → Calendar & Agenda → 农历** 开关。开启或关闭该开关会同时为两个日历添加或移除相同的紧凑农历注释；只读节日日历覆盖也保持一致。日历固定在可独立滚动的时间线上方；下方 Grabber 每次完成拖动只切换一个相邻尺寸，继续拉动时步进会逐渐减小而不是突然停住。Week 左右拖动会按距离在五列事件日期轨道内吸附，Month 和 Year 左右滑动才切换上一或下一完整周期并使用与 Agenda 相同的方向动画；开启 Reduce Motion 后会到达相同结果但减少空间运动。有日志的日期会显示标记；点击有标记的日期可把时间线移动到当天，没有标记的日期只用于浏览，不会替换当前选中的真实 Journal 日。滚动时间线会同步日历选中日期和当前显示周期；如果选中日仍然存在，后台刷新也会保留用户另外浏览到的日历周期。时间线沿用 Agenda 的紧凑日期标题和细分隔线，直接显示时间、标题与元数据；每日文件名不再重复占一行。

Week 不是普通七日页，而是和 Agenda 共用的五列事件日期轨道。它按已有日记日期和外部全天日历日期排列，并在内容边界外保留相邻空日期；一次松手最多跨四列，只吸附一次，不会再叠加整页切换或弹簧回弹。Week 中的空日期也只移动当前浏览位置，不会清除最后一个有真实来源的 Journal 选中日。Month 和 Year 才按完整周期翻页。iPad 横屏左栏沿用同一五列轨道，点到空日期后右侧仍显示此前选中日期的真实日志内容。
