...
Week 不是普通七日页，而是和 Agenda 共用的五列事件日期轨道。它按已有日记日期和外部全天日历日期排列，并在内容边界外保留相邻空日期；一次松手最多跨四列，只吸附一次，不会再叠加整页切换或弹簧回弹。Month 和 Year 才按完整周期翻页。iPad 横屏左栏沿用同一五列轨道，右侧仍显示选中日期的日志内容。
