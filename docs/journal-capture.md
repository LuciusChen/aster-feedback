...
Journal 与 Agenda 共用同一个会持久化的 Week/Month/Year 日历尺寸，也共用 **设置 → Calendar & Agenda → 农历** 开关。开启或关闭该开关会同时为两个日历添加或移除相同的紧凑农历注释；只读节日日历覆盖也保持一致。日历固定在可独立滚动的时间线上方；下方 Grabber 每次完成拖动只切换一个相邻尺寸，继续拉动时步进会逐渐减小而不是突然停住。Week 每次有效左右松手固定切换五个日历日，Month 每次切换一个月，Year 每次切换一年；三者使用相同的整页方向切换动态，iOS Reduce Motion 或 Android 系统动画辅助设置会减少空间运动但不改变结果。Journal 初次打开会和 Agenda 一样高亮今天，即使最近一篇日志早于今天；时间线和横向大屏详情仍从最近一篇真实日志开始。有日志的日期会显示标记；点击有标记的日期可把时间线移动并选中到当天，没有标记的日期只用于浏览，不会替换当前选中的真实 Journal 日。明确选择或继续滚动时间线后，日历选中日期和当前显示周期会保持同步；如果选中日仍然存在，后台刷新也会保留用户另外浏览到的日历周期。时间线沿用 Agenda 的紧凑日期标题和细分隔线，直接显示时间、标题与元数据；每日文件名不再重复占一行。

Week 不是普通七日页，而是和 Agenda 共用的五列事件日期轨道。它按已有日记日期和外部全天日历日期排列，并在内容边界外保留相邻空日期；一次有效松手固定切换五个日历日，并使用和 Month、Year 相同的单次整页方向动画，不会再叠加第二次吸附或弹簧回弹。Week 中的空日期也只移动当前浏览位置，不会清除最后一个有真实来源的 Journal 选中日。iPad 横屏左栏和 Android 平板 Navigation Rail 右侧的 Journal 左栏沿用同一五列轨道；点到空日期后，右侧仍显示此前选中日期的真实日志内容。
...
从 iOS/iPadOS build 11 起，与日记无关的工作区更新会复用 Journal 的阅读预览和搜索内容，重新打开搜索也不会重复准备。日记源文件、参与显示的文件或历史范围发生变化时仍会更新，包括跨天后的日期范围变化。

Journal 是忠实呈现源文件的阅读视图，不采用 Agenda 的语义投影规则。日期标题下面的每个直接子标题只建立一个时间线根节点，并在其中保留完整的 Org 子树。

例如，`** What I Consume? :consume:` 和它下面的 `*** 《好雨知时节》这个短剧不错！` 都会按原层级显示。Aster 不会把父标题当作 Agenda Container 隐藏，也不会把子标题重复提升成另一个顶层时间线条目。对于只有日期标记、日期标题或 Denote 日期文件名但没有日期根标题的文件，每个一级标题成为一个时间线根节点。

iOS/iPadOS build 12 中，轻点标题保留阅读状态；长按标题，再选择编辑，才打开它在源文件中的准确位置。这同样适用于子标题和 Org Preview。轻点已有 tag 徽标，或无 tag 时的添加 tag 图标，仍会直接打开带工作区建议的 tag 编辑器，关闭时写回该标题。

## 从哪里新建日志条目

在 Journal 轻点“新建”，会打开已经配置的日志条目模板；没有模板时，Aster 会引导到 **设置 → 新建与存储 → 日志条目模板**。Journal 设置中也有同一入口。
模板保持与 Emacs 一致的“先选择、再填写”顺序，并以紧凑列表行显示；行内 Org 预览没有额外卡片背景。选择模板后，页面标题统一为 **New Journal Entry**，模板名称和 Today's Journal 只作为一行安静的上下文。主要的多行编辑器会占用键盘上方可用的写作空间，不会在一行输入框下面留下无法输入的空白。填写页不会永久占用 Preview 或空白 Attachments 区域；照片、相机、文件、Preview 和有效草稿才会出现的 Add 共用键盘上方的一行工具栏。点击 Preview 会收起键盘并把展开结果移动到可见位置，再次编辑任意运行时字段会隐藏 Preview。选择附件后，内容区才显示可移除的附件列表。

每次选择模板都会开始一份独立草稿，因此同一个模板可以同时有多份未完成条目。返回后，这些条目会出现在单独的 **草稿** 分区。关闭 Journal Capture 时如果仍有未保存的修改，Aster 会询问保存草稿、丢弃更改，还是继续编辑。已保存草稿会保留文字、提示回答、创建时的模板/来源快照，以及私有附件副本；关闭弹窗或重启 App 后仍可继续。成功添加条目只会移除对应的那份草稿。iPhone、iPad 和 Android 都以屏幕上的 Add 作为唯一提交操作，不要求键盘快捷键或外接键盘。
...
如果标题原本是文件最后一行，Aster 也会先换行，再创建
`:PROPERTIES:` 属性抽屉。这样 `ID` 能被 Org 正确识别，Journal 不会把
属性语法当作正文，也能从对应目录加载图片。

附件与文本在同一次 Add 中处理；如果源文件写入失败，本次新复制的附件不会被假装为成功结果。没有选择附件时，填写页只保留工具栏入口，不显示空的附件区块。
