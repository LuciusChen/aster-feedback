...
下一版 iOS/iPadOS（未发布）会在与日记无关的工作区更新后复用 Journal 的阅读预览和搜索内容，重新打开搜索也不会重复准备。日记源文件、参与显示的文件或历史范围发生变化时仍会更新，包括跨天后的日期范围变化。
...
下一版 iOS/iPadOS（未发布）中，轻点标题保留阅读状态；长按标题，再选择编辑，才打开它在源文件中的准确位置。这同样适用于子标题和 Org Preview。轻点已有 tag 徽标，或无 tag 时的添加 tag 图标，仍会直接打开带工作区建议的 tag 编辑器，关闭时写回该标题。
