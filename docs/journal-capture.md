...
每次选择模板都会开始一份独立草稿，因此同一个模板可以同时有多份未完成条目。返回后，这些条目会出现在单独的 **草稿** 分区。关闭 Journal Capture 时如果仍有未保存的修改，Aster 会询问保存草稿、丢弃更改，还是继续编辑。已保存草稿会保留文字、提示回答、创建时的模板/来源快照，以及私有附件副本；关闭弹窗或重启 App 后仍可继续。成功添加条目只会移除对应的那份草稿。iPhone 和 iPad 都以屏幕上的 Add 作为唯一提交操作，不要求 `⌘ Return` 或外接键盘。
