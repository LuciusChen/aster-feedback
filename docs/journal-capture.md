...
月份日历固定在可独立滚动的时间线上方。有日志的日期会显示标记；点击日期可把时间线移动到当天，滚动时间线也会同步日历选中日期，并以动画切换月份。时间线沿用 Agenda 的紧凑日期标题和细分隔线，直接显示时间、标题与元数据；每日文件名不再重复占一行。Week/Month/Year 仍只在设置中限制读取范围，不会成为 Journal 页面的第二套切换控件。
