...
Journal 是忠实呈现源文件的阅读视图，不采用 Agenda 的语义投影规则。日期标题下面的每个直接子标题只建立一个时间线根节点，并在其中保留完整的 Org 子树。例如，`** What I Consume? :consume:` 和它下面的 `*** 《好雨知时节》这个短剧不错！` 都会按原层级显示；Aster 不会把父标题当作 Agenda Container 隐藏，也不会把子标题重复提升成另一个顶层时间线条目。对于只有日期标记、日期标题或 Denote 日期文件名但没有日期根标题的文件，每个一级标题成为一个时间线根节点。轻点任意可见 Journal 标题会打开它在源文件中的准确位置；轻点已有 tag 徽标，或无 tag 时的添加 tag 图标，会打开带工作区建议的 tag 编辑器，关闭时写回该标题。
