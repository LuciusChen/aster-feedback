...
iOS/iPadOS build 16 supports [selective folder sync](sync-and-safety.md). Existing local files in excluded folders still open; cloud-only files ask you to re-enable their folder in Settings first. Explicitly confirmed cloud file operations in Files remain separate from automatic sync scope.
...
iOS/iPadOS build 15 recognizes tabs and closing `#` markers in Markdown headings. A plain line without a pipe inside a GFM table remains a body cell until a blank line or another block ends the table. Code immediately following a table stays separate, whether indented with tabs or equivalent spaces. Source text remains unchanged.
...
Before adding an attachment, iOS/iPadOS build 15 checks that the actual destination stays inside the workspace, including existing symlinked directories. An escaping destination produces an error before copying. This does not require UUIDs; safe custom Org IDs remain valid.
