...
Divergent edits to the same item stop sync and identify the reminder to review. This also applies when two copies already differ before the first baseline exists. Compare that item in `apple-reminders.org` and the Reminders app, make the fields you want to keep agree, and sync again; repeated retries alone cannot resolve it.

When an old identifier disappears, an unchanged local item may follow the system snapshot. A locally edited item is preserved for review. The last successful baseline is device-local sync metadata, not another editable task file.
