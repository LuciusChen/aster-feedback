...
**iOS/iPadOS build 16：** 状态编辑页不再显示 Shortcut，状态列表也不再显示 Key 提示。括号里的 `t`、`w` 等是 Emacs 快捷键，不需要在手机上填写。已有快捷键仍会随状态配置保留，修改名称、类型、日志或外观不会清除它们。

流程概览中的 Process、Terminal 关键词较多时会自动分行排列，单个关键词不拆行，顺序不变。仍可点整张卡片进入编辑。
...
时间线用一条紧凑的七日节奏帮助快速扫读。详情保留 28 天历史，但 28 个小色块只占一行，不再展开成四行面板。图例、连续天数和 28 日完成率仍然可见。
